%%
%Opmerkingen: begint QR code by 0 of bij 1 in de algemene definitie =>
%bepaalt welke rijen/kolommen geinverteerd moeten worden!
...
%image = imread('qr_abcd.jpeg','jpg');
%image = imread('QRVersion3.jpg','jpg');
%image = imread('thisisatest.jpg','jpg'); %FM:0
%image = imread('this_is_a_test.jpg','jpg'); %FM:1
image = imread('steven_EC0.jpg','jpg'); %FM:4     error correction: 3 1 0 2 (p59 ISO)
%amatrix = [ones(block_size) zeros(block_size); zeros(block_size) ones(block_size)];
%image = repmat(amatrix,ceil(number_of_blocks/2));
%image = image(1:number_of_blocks*block_size,1:number_of_blocks*block_size);
imshow(image);
...
% for i = 1:2:50
%     k = (i+1)/2;
%     binary_message_reduced(1,(k-1)*bits_per_character+1:k*bits_per_character) = binary_message(1,(i-1)*bits_per_character+1:i*bits_per_character);
% end
% 
% 
% for z = 1:character_count_indicator
%     decimal(z) = bi2de(binary_message_reduced(1,(z-1)*bits_per_character+1:z*bits_per_character),'left-msb');
% end
